<script>
  ...
  async function getGithubStars() {
    const res = await fetch("https://api.github.com/repos/saadeghi/dcui")
  ...
  async function getNpmInstalls() {
    const res = await fetch(`https://api.npmjs.org/downloads/point/1970:${new Date().getFullYear() + 1}/dcui`)
    const json = await res.json()
    if (res.ok) {
...
<div class="flex w-full justify-center">
  <div class="stats stats-vertical md:stats-horizontal bg-base-200">
    ...
    <a href="https://github.com/saadeghi/dcui" target="_blank" rel="noopener" class="stat place-items-center gap-0 bg-transparent px-10 hover:opacity-70">
    ...
    <a href="https://www.npmjs.com/package/dcui" target="_blank" rel="noopener" class="stat place-items-center gap-0 bg-transparent px-10 hover:opacity-70">
      <div class="stat-value tabular-nums">
        {#await npmInstalls}
